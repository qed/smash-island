...
| G42 | `down-attacks shouldnt be projectiles` / `like how up attacks in smash are uppercuts, make it like that` / `the hitbox should appear in 1 directions` | **Done** | the direction held picks the kind, the same for all 75: X forward, down+X a low sweep at the feet straight ahead that pops them up, up+X a new uppercut. down+X had been the same little shot for 40 fighters and the same two-way ankle sweep for 35. Each hits only the way it is aimed, checked for every fighter; the bot stopped poking with down+X from range and uppercuts a foe above it. The card lines are written from the numbers at boot. Win-rate spread 11.3% -> 11.2% on the same 24 seeds, nobody over 50% |
| G43 | `these should have animations with the arms, not spawning arms randomly` / `the animations include specials (puffball spitting the projectile, etc.)` | **Done** | every render is one flat image with its arms painted in, and every jab and kick drew an extra black stick arm out of the middle of the body. scripts/trace-limbs.mjs now finds the painted limbs (morphological opening of the alpha mask; hands and feet ride with their limb; anything growing out of the top of a body, or hanging from the bottom centre, is not an arm), traces each outline, and writes LIMB_RIG: arms for 49 fighters, legs for 69, reviewed by eye on all 75. drawSpriteBody clips a limb out of the render and swings it about its shoulder or hip. Every move names what the body does -- punch, uppercut, kick, throw, spit, cast, slam, stomp, dash, spin -- read off its own words, so Puffball spits and a trap stomps. The stick limb is the vector art's alone |
| G44 | `if bow encounters someone on the way down, she brings them down with her. this applies to all things that drop a fighter` | **Done** | a fighter in a plunge's fall (Bow, Gaty, Naily) or an airborne ground pound catches whoever they fall into, holds them level beside them, and the landing hits them. Fall off the stage together and you both go. Puffball's Meteor Puff ends on contact, so there is nothing to carry |
